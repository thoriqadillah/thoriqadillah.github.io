...
## The Motivation
Eksekutif Mahasiswa Sistem Informasi (EMSI) UB adalah organisasi yang menaungi mahasiswa program studi sistem informasi di UB. Every year, EMSI has several events that require volunteers so that the event can run. In the process of recruiting volunteers, of course, a recruitment process is needed. In the recruitment process, EMSI utilizes their website called WANGSIT to manage and simplify the open recruitment process. 
